...
function createIconMarker(Icon: ComponentType<{ className?: string; style?: CSSProperties; 'aria-hidden'?: boolean }>, color: string, label: string, large = false) {
  ...
  element.className = `pointer-events-none flex items-center justify-center rounded-full border-2 border-background text-background shadow-lg ${large ? 'size-14' : 'size-7'}`
...
function distanceLabel(distance?: number) {
  if (distance == null) return 'Adicionado pelo corretor'
  return distance < 1000 ? `${distance} m` : `${(distance / 1000).toFixed(1).replace('.', ',')} km`
}
...
export function PropertyLocationMap({ latitude, longitude, title, customPoints }: Props) {
  ...
  useEffect(() => {
    ...
    new mapboxgl.Marker({ element: propertyMarker, anchor: 'center' }).setLngLat([longitude, latitude]).addTo(map)
    mapRef.current = map
    return () => {
  ...
  useEffect(() => {
    ...
    markersRef.current = points.map((point) => {
      const iconMarker = createIconMarker(categoryIcons[point.category], markerColors[point.category], `${NEARBY_CATEGORIES[point.category]}: ${point.name}`)
  ...
  if (!process.env.NEXT_PUBLIC_MAPBOX_ACCESS_TOKEN) {
    return <div className="rounded-2xl bg-muted p-8 text-center text-sm text-muted-foreground">O mapa está temporariamente indisponível.</div>
  }
              ...
              {points.slice(0, 10).map((point) => (
                <li key={point.id} className="flex items-center gap-3 rounded-xl border border-border p-3">
                  ...
                  <span className="min-w-0">
                    ...
                    <span className="block text-xs text-muted-foreground">{NEARBY_CATEGORIES[point.category]} · {distanceLabel(point.distance)}</span>
                  </span>
                </li>
              ))}
            </ul>
          ) : (
            <p className="text-sm text-muted-foreground">Nenhum ponto próximo encontrado nesta região.</p>
          )}
        </div>
      </div>
    </section>
